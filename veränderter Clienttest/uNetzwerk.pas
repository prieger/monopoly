...
//Straßen-ID soll übergeben werden? ohne Parameter
   ...
   if action = 'baue' then Monopoly.baue(CutPayload2(Payload).erstes, CutPayload2(Payload).zweites);
...
//Neue ClientServer: ob der Spieler Karten einsetzen kann     -    was nun?
   ...
   if action = 'hndl' then Spieler.hndl;
...
//ClientServer(neue Version): Spieler soll handlung angeben     -   was nun? (gleiche aktion gemeint - oder neuer Typus)
   ...
   if action = 'koau' then Spieler.koau;
    ...
    if action = 'spin' then Spieler.spin(CutPayload4(Payload).erstes, CutPayload4(Payload).zweites, CutPayload4(Payload).drittes);
...
//welche ID? und Name des Spielers ID soll gemeint sein, welcher Spieler zu der ID gehört?
    ...
    if action = 'szug' then Spieler.szug(CutPayload1(Payload).erstes, CutPayload1(Payload).zweites);
